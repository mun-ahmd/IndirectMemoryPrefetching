...
void CmdLineParser::printExplanationPretty(const CmdLineOption *option,
  ...
  os << nameAndArg;
  if (nameAndArg.size() < prefixWidth - 3) {
    os << ' ';
  }
  ...
  while ((start < end) && (explanation[start] == ' ')) {
    start++;
  }
  while ((start < end) && (explanation[end] == ' ')) {
    end--;
  }
  ...
  while (expl.size() - position > lineWidth - prefixWidth) {
    ...
    unsigned long searchStart =
        min((unsigned long)position + lineWidth - prefixWidth,
...
int CmdLineParser::calculatePrefixColumnWidth() const {
  ...
  for (unsigned int i = 0; i < allOptions.size(); i++) {
    prefixWidth =
        max(prefixWidth, (int)prettyOptionNameAndArg(allOptions[i]).size());
  }
  return prefixWidth;
}
