...
public:
   ...
   */
  static void sizeOfFVFile(FVParser &parser, int *numPoints, int *numDims);
   ...
   */
  static void loadAndProjectFVFile(FVParser &parser, const Dataset &projection,
                                   Dataset *result);
...
/* To achieve the same behavior from the random number generator, we implement
 * our own random number generator rather than rely on the system's random
 * number generator. The randFloat method implementation is taken from ran2 in
 * "Numerical Recipes in C" -- see
