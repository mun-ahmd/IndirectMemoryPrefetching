...
void Utilities::sizeOfFVFile(FVParser &parser, int *numPoints, int *numDims) {
  ...
  *numDims = 0;
  ...
  while (parser.nextLine(&tokens)) {
    for (list<FVParserToken>::iterator i = tokens.begin(); i != tokens.end();
         i++) {
...
void Utilities::randomProjectionMatrix(int randSeed, Dataset *projection) {
  check(NULL != projection,
        "Utilities::randomProjectionMatrix() projection is null");
...
void Utilities::loadFVFile(FVParser &parser, Dataset *result) {
  ...
  while (parser.nextLine(&current_vector)) {
    ...
    if (point >= result->numRows()) {
      check(false, "Utilities::loadFVFile() more vectors than expected "
                   "(loading vector " +
                       toString((int)(point + 1)) + " when expecting " +
    ...
    for (i = current_vector.begin(); i != current_vector.end(); i++) {
      sumVector += i->value;
      if (i->dimension > largestDimensionSeen) {
        largestDimensionSeen = i->dimension;
      }
    }
    ...
    for (i = current_vector.begin(); i != current_vector.end(); i++) {
      if (i->dimension > (int)result->numCols() || i->dimension < 1) {
        ...
        check(i->dimension >= 1, "Utilities::loadFVFile() dimension < 1");
      }
...
void Utilities::loadAndProjectFVFile(FVParser &parser,
  ...
  while (parser.nextLine(&current_vector)) {
    ...
    if (point >= (int)result->numRows()) {
      check(false,
            "Utilities::loadAndProjectFVFile() more vectors than expected "
            "(loading vector " +
                toString(point + 1) + " when expecting " +
                toString((int)result->numRows()) + ")");
    }
    ...
    for (i = current_vector.begin(); i != current_vector.end(); i++) {
      ...
      if (dim > largestDimensionSeen) {
        largestDimensionSeen = dim;
      }
      if (dim < 1 || dim > numProjectedRows) {
        ...
        check(dim <= numProjectedRows,
              "Utilities::loadAndProjectFVFile() expecting only " +
                  ...
                  " dimensions, but found dimension " + toString((int)dim));
      }
