...
void CACHE::l1d_prefetcher_initialize() { 
    std::cout << NAME << " Tyche_custom DCache prefetcher" << "cpu: " << cpu << std::endl; 
...
void CACHE::prefetcher_cycle_operate()
{
    ...
    dct[cpu].check_state();
#endif
    agq[cpu].flush_stale_requests(current_cycle);
    auto agq_item = agq[cpu].priority_item();
...
void search_ima(uint32_t cpu, uint64_t ip, uint64_t pf_address, uint64_t current_cycle, uint64_t origin_addr, int64_t delta) {
    ...
    if(dct_ptr != -1 && dct[cpu].has_successor(dct_ptr) && dct[cpu].buffer[dct_ptr].head){
        ...
        new_agq_item.priority = dct[cpu].buffer[dct_ptr].conf;
        new_agq_item.cycles_inserted = current_cycle;
        new_agq_item.ret_value = pf_address;
        // Tiered Aging based on confidence counters
        if (new_agq_item.priority >= 3) {
    new_agq_item.max_age = 200; // Keep the "VIP" items
} else {
    // Drop Priority 2 items MUCH faster to prevent AGQ_FULL
    new_agq_item.max_age = 50; 
}
    #ifdef PREFETCH_DEBUG
        new_agq_item.cycle = current_cycle;
    #endif
        agq[cpu].insert_priority(new_agq_item);
    }
