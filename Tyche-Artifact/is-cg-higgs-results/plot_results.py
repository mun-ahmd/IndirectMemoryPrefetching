...
Compares Tyche and IMP vs no prefetcher (baseline = 1.0 for IPC).
"""
...
def main():
    ...
    prefetchers = ["no_prefetcher", "tyche", "imp"]
    ...
    for task in tasks:
        ...
        for p in prefetchers:
            row = next(
                (r for r in rows if r["Task"] == task and r["Prefetcher"] == p), None
            )
            if row:
    ...
    ax1 = axes[0, 0]
    bars0 = ax1.bar(
        x - width, data["no_prefetcher"], width, label="no prefetcher", color="gray"
    )
    bars1 = ax1.bar(x, data["tyche"], width, label="Tyche", color="C0")
    bars2 = ax1.bar(x + width, data["imp"], width, label="IMP", color="C1")
    ...
    req = {
        p: [
            next(
                (
                    r["L1D_Prefetch_Requests"]
                    for r in rows
                    if r["Task"] == t and r["Prefetcher"] == p
                ),
                0,
            )
            for t in tasks
    ...
    ax2.bar(x + width, req["imp"], width, label="IMP", color="C1")
    ...
    hr = {
        p: [
            hit_rate(
                next((r for r in rows if r["Task"] == t and r["Prefetcher"] == p), _def)
            )
            for t in tasks
        ]
        for p in prefetchers
    ...
    ax_hr.bar(
        x - width, hr["no_prefetcher"], width, label="no prefetcher", color="gray"
    )
    ax_hr.bar(x, hr["tyche"], width, label="Tyche", color="C0")
    ax_hr.bar(x + width, hr["imp"], width, label="IMP", color="C1")
    ...
    ax3.bar(
        x - width, hits["no_prefetcher"], width, label="no prefetcher", color="gray"
    )
    ax3.bar(x, hits["tyche"], width, label="Tyche", color="C0")
    ax3.bar(x + width, hits["imp"], width, label="IMP", color="C1")
    ...
    ax4.bar(
        x - width, miss["no_prefetcher"], width, label="no prefetcher", color="gray"
    )
    ax4.bar(x, miss["tyche"], width, label="Tyche", color="C0")
    ax4.bar(x + width, miss["imp"], width, label="IMP", color="C1")
