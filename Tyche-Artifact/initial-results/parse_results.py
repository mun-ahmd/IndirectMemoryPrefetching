...
def parse_prefetcher_from_filename(name: str) -> str:
    ...
    if name.startswith("results_no_crono"):
        return "no_prefetcher"
    if name.startswith("results_tyche_crono"):
        return "tyche"
    if name.startswith("results_ipcp_l1d_crono"):
        return "ipcp_l1d"
    return "unknown"
...
def parse_result_file(path: Path) -> dict | None:
    ...
    requested_match = re.search(
        r"cpu0_L1D PREFETCH\s+REQUESTED:\s*(\d+)", text
    )
