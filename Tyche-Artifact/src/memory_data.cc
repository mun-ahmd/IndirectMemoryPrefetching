...
int64_t MEMORY_DATA::read(uint64_t addr, uint8_t size, bool sign) {
  ...
  case SIZE_WORD:
    ...
    ret_data =
        sign ? ((int32_t)ram_data_w) : (((uint64_t)ram_data_w) & 0xffffffff);
    break;
  case SIZE_DWORD:
...
void MEMORY_DATA::write(uint64_t addr, uint64_t wdata, uint8_t size) {
  ...
  // [FIX 1] Handle Size 0 (Prefetches/Cache Ops) gracefully
  if (size == 0) {
    return;
  }
  ...
  case SIZE_HWORD:
    ...
    *((uint16_t *)(ram + addr)) = ram_wdata_h;
    break;
  case SIZE_WORD:
    // [CRITICAL BUG FIX DETECTED BELOW]
    ram_wdata_w = (uint32_t)(wdata & 0xffffffff); // Was 0xffff (Bug!)
