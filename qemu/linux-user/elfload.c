...
void qemu_dump_guest_memory(const char* filename) {
    ...
    for (vma = vma_first(mm); vma != NULL; vma = vma_next(vma)) {
        // [PATCH START] ----------------------------------------------- (author: mun-ahmd)
        // Filter out QEMU Host Memory (High Addresses > 512GB)
        // LoongArch Guest (0x120...) -> KEEP
        // x86 Host QEMU (0x555... or 0x7fff...) -> SKIP
        if (vma->vma_start > 0x0000008000000000ULL) {
            continue;
        }
        // [PATCH END] -------------------------------------------------
