...
static int loongarch_map_tlb_entry(CPULoongArchState *env, hwaddr *physical,
  ...
  // Commented to support 14 bit page size, see
  // https://github.com/qemu/qemu/commit/1d832c19db1e895b7d8015c3b33fc6311e20a69c#diff-e53ba01f35391b7bee93bdda3aaf013bf40c83fdeb2a4d1773bdd5d33fb29add
  // (mun-ahmd) tlb_ppn = tlb_ppn & ~(((0x1UL << (tlb_ps - 12)) -1));
...
static int loongarch_map_address(CPULoongArchState *env, hwaddr *physical,
  ...
  if (match) {
    return loongarch_map_tlb_entry(env, physical, prot, address, access_type,
                                   index, mmu_idx);
  }
...
static void raise_mmu_exception(CPULoongArchState *env, target_ulong address,
  ...
  if (tlb_error == TLBRET_NOMATCH) {
    ...
    if (is_la64(env)) {
      env->CSR_TLBREHI = FIELD_DP64(env->CSR_TLBREHI, CSR_TLBREHI_64, VPPN,
                                    extract64(address, 13, 35));
    } else {
      env->CSR_TLBREHI = FIELD_DP64(env->CSR_TLBREHI, CSR_TLBREHI_32, VPPN,
...
void helper_invtlb_page_asid(CPULoongArchState *env, target_ulong info,
  ...
  for (int i = 0; i < LOONGARCH_TLB_MAX; i++) {
    ...
    if (!tlb_g && (tlb_asid == asid) && (vpn == (tlb_vppn >> compare_shift))) {
      tlb->tlb_misc = FIELD_DP64(tlb->tlb_misc, TLB_MISC, E, 0);
    }
  }
  tlb_flush(env_cpu(env));
...
void helper_ldpte(CPULoongArchState *env, target_ulong base, target_ulong odd,
  ...
  if (huge) {
    ...
    /* Move Global bit */
    tmp0 = ((tmp0 & (1 << LOONGARCH_HGLOBAL_SHIFT)) >> LOONGARCH_HGLOBAL_SHIFT)
               << R_TLBENTRY_G_SHIFT |
           (tmp0 & (~(1 << R_TLBENTRY_G_SHIFT))); // To support 16 KB page size
                                                  // again (mun-ahmd)
    ps = ptbase + ptwidth - 1;
    if (odd) {
