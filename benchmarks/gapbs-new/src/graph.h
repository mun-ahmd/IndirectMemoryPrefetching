  ...
  class Neighborhood {
   ...
   public:
    Neighborhood(NodeID_ n, DestID_** g_index, OffsetT start_offset) :
      ...
      // NT: g_index is the pointer to vertexlist
      // just NideID in case of unweighted graphs
      // and NodeWeight in case of weighted graphs
      OffsetT max_offset = end() - begin();
      start_offset_ = std::min(start_offset, max_offset);
      //std::cout << "vertexlist = " << g_index << std::endl;
      //std::cout << "edgelist = " << *g_index_ << std::endl;
      //std::cout << "inside Neighborhood function for n = " << n << std::endl;
      //std::cout << "end() = " << end() << ", begin() = " << begin() << ", max_offset = " << max_offset << std::endl;
    }
