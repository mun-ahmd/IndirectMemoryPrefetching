...
pvector<WeightT> DeltaStep(const WGraph &g, NodeID source, WeightT delta) {
  ...
  pf_params_t* params = new pf_params_t(4, 4, 1, omp_get_max_threads());
  pf_enable_t* enable = new pf_enable_t();
  pf_params_t& pf_p = *params;

  // Nodes in the DIG
  pf_p.RegisterNode(&(*frontier.begin()), g.num_edges_directed(), 0);
  pf_p.RegisterNode(g.out_index_, g.num_nodes()+1, 1);
  pf_p.RegisterNode(g.out_neighbors_, g.num_edges(), 2);
  pf_p.RegisterNode(&(*dist.begin()), g.num_nodes(), 3);

  //Edges for traversal
  pf_p.RegisterTravEdge(0, 1, BaseOffset_int32_t);
  pf_p.RegisterTravEdge(0, 3, BaseOffset_int32_t);
  pf_p.RegisterTravEdge(1, 2, PointerBounds_uint64_t);
  pf_p.RegisterTravEdge(2, 3, BaseOffset_int32_t);

  // Edges for triggers
  pf_p.RegisterTrigEdge(0, 0, UpToOffset, SquashIfLarger);
